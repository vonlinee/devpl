<template>
  <div class="number-range" :class="{ 'is-disabled': disabled, 'is-focus': isFocus }">
    <el-input-number :disabled="disabled" placeholder="最小值" @blur="handleBlur" @focus="handleFocus"
      @change="handleChangeMinValue" v-model="minValue_" v-bind="$attrs" v-on="['update:minValue']" :controls="false" />
    <div class="to">
      <span>{{ to }}</span>
    </div>
    <el-input-number :disabled="disabled" placeholder="最大值" @blur="handleBlur" @focus="handleFocus"
      @change="handleChangeMaxValue" v-model="maxValue_" v-bind="$attrs" v-on="['update:maxValue']" :controls="false" />
    <div id="append">
      <slot name="append">
        <!-- 后缀插槽 -->
      </slot>
...
const handleChangeMinValue = (value: number) => {
  ...
  // min > max 交换min max
  if (typeof newMinValue === 'number' && parseFloat(String(newMinValue)) > parseFloat(String(props.maxValue))) {
    // 取值范围判定
...
const handleChangeMaxValue = (value: number) => {
  ...
  // max < min 交换min max
  if (typeof newMaxValue === 'number' && parseFloat(String(newMaxValue)) < parseFloat(String(props.minValue))) {
    // 取值范围判定
