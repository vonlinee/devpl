<template>
  <div>
    <el-table v-loading="options.loading" :element-loading-text="options['element-loading-text']"
              :element-loading-spinner="options['element-loading-spinner']"
              :element-loading-svg="options['element-loading-svg']"
              :element-loading-background="options['element-loading-background']" ref="tableRef" class="el-table"
              :data="tableDataRef" :height="options.height" :max-height="`${options['max-height']}px`"
              ...
              :tooltip-effect="options['tooltip-effect']"
              :show-summary="options['show-summary']"
              :sum-text="options['sum-text']"
              :select-on-indeterminate="options['select-on-indeterminate']"
...
const { columns, options, api, form } = config;

// 窗口显示状态
const modalVisiable = ref(false);
// 当前操作的行数据
const currentRow: RowDataModel = ref();
// 表单对象 和当前行的数据关联
