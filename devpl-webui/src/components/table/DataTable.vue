<template>
	<div>
		<el-table v-loading="options.loading" :element-loading-text="options['element-loading-text']"
			:element-loading-spinner="options['element-loading-spinner']" :element-loading-svg="options['element-loading-svg']"
			:element-loading-background="options['element-loading-background']" ref="tableRef" class="el-table"
			:data="tableData" :height="options.height" :max-height="`${options['max-height']}px`" :stripe="options.stripe"
			:border="options.border" :size="options.size" :fit="options.fit" :show-header="options['show-header']"
			:current-row-key="options['current-row-key']" :highlight-current-row="options['highlight-current-row']"
			:empty-text="options['empty-text']" :row-key="options['row-key']" :row-class-name="options['row-class-name']"
			:cell-class-name="options['cell-class-name']" :tooltip-effect="options['tooltip-effect']"
			:show-summary="options['show-summary']" :sum-text="options['sum-text']"
			:select-on-indeterminate="options['select-on-indeterminate']" :indent="options.indent" :lazy="options.lazy"
			:load="options.load" :tree-props="options['tree-props']" @select="select" @select-all="selectAll"
		...
		<vxe-modal v-model="modalVisiable" :title="modalTitle" width="800" height="600" show-footer>
			...
			<template #footer>
				<div>
					...
					<el-button type="success" @click="setModalVisiable(false)">确定</el-button>
				</div>
			</template>
		</vxe-modal>
...
import { PropType, onMounted, ref, toRaw } from "vue";
...
// 表格行数据模型
type RowModel = Record<string, any>
...
export interface DataTableApiConfig {
	...
	// 删除单条记录
	deleteOne: (row: RowModel) => Promise<any>,
	// 更新单条记录
	update: (row: RowModel) => Promise<any>,
}
 ...
 * DataTable表单配置
 */
interface DataTableFormOptions {
	/**
	 * 表单数据对象，用于绑定表单数据项的值
	 */
	 ...
	 */
	editConverter: (row: RowModel, formData: FormDataModel) => FormDataModel,
	/**
	 * 重置表单，弹窗关闭时进行重置表单
...
// 当前操作的行数据
const currentRow: RowModel = ref();
// 表单对象 和当前行的数据关联
const formObject = ref(props.formObject)
// 分页相关数据
const pageSizeList = [10, 20, 30, 40]
// 当前页码
const currentPageIndex = ref(5)
// 当前每页数据条数
...
const setModalVisiable = (val: boolean) => {
	...
	if (!val) {
		if ("新增" == modalTitle.value) {
			...
			props.apiConfig?.update(currentRow.value)
		}
	}
};
...
const refreshTableData = (pageIndex: number, pageSize: number) => {
	console.log("refresh ", pageIndex, pageSize);

	props.apiConfig?.queryPage(pageIndex, pageSize, null).then((res => {
		if (res.code == 200) {
...
const handleEditOperation = (index: number, row: any) => {
	...
	let newformObject = props.convertRowToForm(toRaw(row), formObject.value);
	if (newformObject) {
...
const handleDelete = (index: number, row: any) => {
	ElMessageBox.confirm(
		...
		.then(() => {
			props.apiConfig?.deleteOne(toRaw(row)).then((res) => {
				if (res.code == 200) {
