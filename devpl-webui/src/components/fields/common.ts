  ...
  function initDragSort(tableBodyDom: HTMLElement, tableData: any[]) {
    setTimeout(() => {
      ...
      const dragEnterTextList =
        tableBodyDom.querySelectorAll<HTMLElement>("tr .dragEnterText")

      const moveIconDoms =
        tableBodyDom.querySelectorAll<HTMLElement>(".moveIcon")
      ...
      for (let i = 0; i < trList.length; i++) {
        ...
        trList[i].ondragenter = (e: DragEvent) => {
          ...
          const dragEnterText = trList[i].querySelector(
            ".dragEnterText"
          ) as HTMLElement
