...
public class ClipConstructor {
    ...
    public XMLNode createClip(Node clip) {
        ...
        if (clip instanceof Polygon polygon) {
        ...
        } else if (clip instanceof Polyline polyline) {
            xmlClip = new XMLNode("polyline");
            appendPoints(polyline, xmlClip);
        } else if (clip instanceof SVGPath path) {
            xmlClip = new XMLNode("path");
            xmlClip.addAttribute("d", path.getContent());
        } else if (clip instanceof Path path) {
            xmlClip = new XMLNode("path");
            setPath(path, xmlClip);
        } else if (clip instanceof Circle circle) {
        ...
        } else if (clip instanceof Ellipse ellipse) {
        ...
        } else if (clip instanceof Arc arc) {
            Path path = new Path();
            ...
            xmlClip = new XMLNode("path");
            setPath(path, xmlClip);
        } else if (clip instanceof Rectangle rec) {
        ...
        } else if (clip instanceof QuadCurve curve) {
            Path path = new Path();
            MoveTo moveTo = new MoveTo();
            ...
            xmlClip = new XMLNode("path");
            setPath(path, xmlClip);
        } else if (clip instanceof CubicCurve curve) {
            Path path = new Path();
            MoveTo moveTo = new MoveTo();
            moveTo.setX(curve.getStartX());
            moveTo.setY(curve.getStartY());
