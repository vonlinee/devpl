...
public class ClassView extends FxmlView {
    ...
    public void generateDDL(ActionEvent actionEvent) {
        ...
        Map<JavaType, TypeMapping> map = CollectionUtils.toMap(TypeMapping.mapppings, TypeMapping::getJavaDataType, Function.identity());

        List<Field> fieldList = new ArrayList<>();
        for (FieldInfo item : items) {
            TypeMapping typeMapping = map.get(item.getDataType());
            if (typeMapping != null) {
                Field field = new Field();
                field.setName(item.getName());
                field.setType(item.getDataType().getQualifier());
                field.setComment(item.getRemarks());
                field.setPrimaryKey(false);
                fieldList.add(field);
            }
        }
        String sql = DdlFormatUtil.buildDdlScript("user", fieldList);
        txaDdlResult.setText(sql);
    }
