...
public class DumpMetadataTask extends Task {
     ...
     */
    public void setConfiguredDatabase(PooledDataSourceWrapper dataSource) {
    ...
    public void setOutputEncoding(Charset encoding) {
        _outputEncoding = encoding.name();
    }
    ...
    public void setTableTypes(String tableTypes) {
        if (tableTypes != null) {
            ...
            String[] tableTypesSplit = tableTypes.split(",");
            for (String tableType : tableTypesSplit) {
    ...
    public void execute() throws DdlUtilsTaskException {
        if (_dataSource == null) {
            throw new DdlUtilsTaskException("No data source specified, so there is nothing to do.");
        }
        OutputStream output;
        if (_outputFile != null) {
            try {
        ...
        try (Connection connection = _dataSource.getConnection()) {
            XmlWriter xmlWriter = new PrettyPrintingXmlWriter(output, _outputEncoding);
            ...
            throw new DdlUtilsTaskException(ex);
        } finally {
            if (_outputFile != null) {
     ...
     */
    private void dumpMetaData(XmlWriter xmlWriter, DatabaseMetaData metaData) throws SQLException {
     ...
     */
    private void dumpProperty(XmlWriter xmlWriter, Object obj, Method propGetter) {
        try {
     ...
     */
    private void addProperty(XmlWriter xmlWriter, String name, Object value) {
        if (value != null) {
     ...
     */
    private void addArrayProperty(XmlWriter xmlWriter, String name, Object[] values) {
     ...
     */
    private void addResultSetProperty(XmlWriter xmlWriter, String name, ResultSet result) {
        String propName = name;
     ...
     */
    private void performResultSetXmlOperation(XmlWriter xmlWriter, String name, ResultSetXmlOperation op) {
     ...
     */
    private void dumpCatalogsAndSchemas(XmlWriter xmlWriter, final DatabaseMetaData metaData) {
     ...
     */
    private void dumpTables(XmlWriter xmlWriter, final DatabaseMetaData metaData) {
     ...
     */
    private void dumpColumns(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String tableName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            ...
            public void handleError(SQLException ex) {
                log("Could not read the columns for table '" + tableName + "' from the result set: " + Arrays.toString(ex.getStackTrace()), Project.MSG_ERR);
            }
        });
    }
     ...
     * @param tableName   The table name
     */
    private void dumpPKs(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String tableName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            @Override
            public ResultSet getResultSet() throws SQLException {
     ...
     * @param tableName   The table name
     */
    private void dumpVersionColumns(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String tableName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            @Override
            public ResultSet getResultSet() throws SQLException {
                    ...
                    if (columns.contains("PSEUDO_COLUMN")) {
                        try {
                            switch (result.getShort("PSEUDO_COLUMN")) {
                                ...
                                default -> xmlWriter.writeAttribute(null, "columnType", "unknown");
                            }
                        } catch (SQLException ex) {
     ...
     * @param tableName   The table name
     */
    private void dumpFKs(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String tableName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            @Override
            public ResultSet getResultSet() throws SQLException {
                ...
                if (columns.contains("UPDATE_RULE")) {
                    try {
                        switch (result.getShort("UPDATE_RULE")) {
                            ...
                            default -> xmlWriter.writeAttribute(null, "updateRule", "unknown");
                        }
                    } catch (SQLException ex) {
                ...
                if (columns.contains("DELETE_RULE")) {
                    try {
                        switch (result.getShort("DELETE_RULE")) {
                            case DatabaseMetaData.importedKeyNoAction, DatabaseMetaData.importedKeyRestrict ->
                            ...
                            default -> xmlWriter.writeAttribute(null, "deleteRule", "unknown");
                        }
                    } catch (SQLException ex) {
                ...
                if (columns.contains("DEFERRABILITY")) {
                    try {
                        switch (result.getShort("DEFERRABILITY")) {
                            ...
                            default -> xmlWriter.writeAttribute(null, "deferrability", "unknown");
                        }
                    } catch (SQLException ex) {
     ...
     * @param tableName   The table name
     */
    private void dumpIndexes(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String tableName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            @Override
            public ResultSet getResultSet() throws SQLException {
                ...
                if (columns.contains("TYPE")) {
                    try {
                        switch (result.getShort("TYPE")) {
                            ...
                            case DatabaseMetaData.tableIndexHashed -> xmlWriter.writeAttribute(null, "type", "hashed");
                            case DatabaseMetaData.tableIndexOther -> xmlWriter.writeAttribute(null, "type", "other");
                            default -> xmlWriter.writeAttribute(null, "type", "unknown");
                        }
                    } catch (SQLException ex) {
     ...
     * @param metaData  The database metadata
     */
    private void dumpProcedures(XmlWriter xmlWriter, final DatabaseMetaData metaData) throws SQLException {
     ...
     */
    private void dumpProcedure(XmlWriter xmlWriter, final DatabaseMetaData metaData, final String catalogName, final String schemaName, final String procedureName) throws SQLException {
        performResultSetXmlOperation(xmlWriter, null, new ResultSetXmlOperation() {
            @Override
            public ResultSet getResultSet() throws SQLException {
                        ...
                        try {
                            switch (result.getInt("NULLABLE")) {
                                ...
                                case DatabaseMetaData.procedureNullable ->
                                    xmlWriter.writeAttribute(null, "nullable", "true");
                                default -> xmlWriter.writeAttribute(null, "nullable", "unknown");
                            }
                        } catch (SQLException ex) {
     ...
     */
    private void addStringAttribute(XmlWriter xmlWriter, String attrName, ResultSet result, Set<String> columns, String columnName) throws SQLException {
     ...
     * @param columnName The name of the column in the result set
     */
    private void addIntAttribute(XmlWriter xmlWriter, String attrName, ResultSet result, Set<String> columns, String columnName) throws SQLException {
        if (columns.contains(columnName)) {
            try {
     ...
     * @param columnName The name of the column in the result set
     */
    private void addShortAttribute(XmlWriter xmlWriter, String attrName, ResultSet result, Set<String> columns, String columnName) throws SQLException {
        if (columns.contains(columnName)) {
            try {
     ...
     * @param columnName The name of the column in the result set
     */
    private void addBooleanAttribute(XmlWriter xmlWriter, String attrName, ResultSet result, Set<String> columns, String columnName) throws SQLException {
        if (columns.contains(columnName)) {
            try {
