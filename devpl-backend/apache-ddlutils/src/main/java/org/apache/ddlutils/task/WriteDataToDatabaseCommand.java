...
public class WriteDataToDatabaseCommand extends ConvertingDatabaseCommand {
    ...
    public void execute(DatabaseTask task, Database model) throws DdlUtilsTaskException {
        if ((_singleDataFile != null) && !_fileSets.isEmpty()) {
            throw new DdlUtilsTaskException("Please use either the datafile attribute or the sub fileset element, but not both");
        }
            ...
            if (_singleDataFile != null) {
                ...
                for (FileSet fileSet : _fileSets) {
                    ...
                    for (String file : files) {
                        readSingleDataFile(task, dataReader, new File(fileSetDir, file));
                    }
                }
            }
        } catch (Exception ex) {
     ...
     */
    private void readSingleDataFile(Task task, DataReader reader, File dataFile) throws DdlUtilsTaskException {
        if (!dataFile.exists()) {
