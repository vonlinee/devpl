...
public class TestSummaryCreatorTask extends Task {
     ...
     */
    protected void addTargetDatabaseInfo(Element element, String jdbcPropertiesFile) throws IOException, DdlUtilsTaskException {
    ...
    private Properties readProperties(String jdbcPropertiesFile) {
        ...
        try {
            ...
            if (propStream == null) {
                ...
                File propFile = new File("", jdbcPropertiesFile);

                if (propFile.exists() && propFile.isFile() && propFile.canRead()) {
                    propStream = new FileInputStream(propFile);
                } else {
                    throw new DdlUtilsTaskException("Cannot load test jdbc properties from file " + jdbcPropertiesFile);
                }
            }
            ...
            throw new DdlUtilsTaskException("Cannot load test jdbc properties from file " + jdbcPropertiesFile, ex);
        } finally {
            if (propStream != null) {
                try {
                    propStream.close();
