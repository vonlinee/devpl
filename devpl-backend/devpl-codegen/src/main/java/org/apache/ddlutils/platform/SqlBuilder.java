...
public abstract class SqlBuilder {
     ...
     */
    private final Logger _log = LoggerFactory.getLogger(SqlBuilder.class);
    ...
    protected void copyData(Table sourceTable, Table targetTable) throws IOException {
        LinkedHashMap<Column, Column> columns = new LinkedHashMap<>();
     ...
     * TODO: Can the format method handle java.sql.Date properly ?
     *
     * @param value 日期和时间类型的值
     * @return 字符串
     */
    protected String formatTime(Object value) {
        DateFormat format = getValueTimeFormat();
        if (format == null) {
            return "";
        }
    ...
    protected void writeColumns(Table table, PojoMap parameters) throws IOException {
        for (int idx = 0; idx < table.getColumnCount(); idx++) {
            ...
            writeColumnDefStmtEnding(table, column, idx, parameters);
        }
    }
    ...
    protected void writeColumnDefaultValueStmt(Table table, Column column) throws IOException {
        ...
        if (parsedDefault != null) {
            if (!getPlatformInfo().isDefaultValuesForLongTypesSupported() && ((column.getTypeCode() == Types.LONGVARBINARY) || (column.getTypeCode() == Types.LONGVARCHAR))) {
    ...
    protected boolean columnsDiffer(Column currentColumn, Column desiredColumn) {
        ...
        boolean sizeMatters = getPlatformInfo().hasSize(currentColumn.getTypeCode()) && (desiredColumn.getSize() != null);
        ...
        return (getPlatformInfo().getTargetJdbcType(desiredColumn.getTypeCode()) != currentColumn.getTypeCode()) || (desiredColumn.isRequired() != currentColumn.isRequired()) || (sizeMatters && !StringUtils.equals(desiredColumn.getSize(), currentColumn.getSize())) || !defaultsEqual;
    }
     ...
     */
    protected void printLineComment(String text) throws IOException {
     ...
     * to use unlimited identifiers; in that case, the identifier is returned unchanged
     */
