...
public class TableServiceImpl extends BaseServiceImpl<TableDao, TableEntity> implements TableService {
    ...
    public void sync(Long id) {
        ...
        List<String> dbTableFieldNameList = dbTableFieldList.stream().map(TableFieldInfo::getFieldName)
            .collect(Collectors.toList());

        // 表字段列表
        List<TableFieldInfo> tableFieldList = tableFieldService.getByTableId(id);

        Map<String, TableFieldInfo> tableFieldMap = tableFieldList.stream()
            .collect(Collectors.toMap(TableFieldInfo::getFieldName, Function.identity()));
        ...
        // 删除数据库表中没有的字段
        List<TableFieldInfo> delFieldList = tableFieldList.stream()
